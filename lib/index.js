(function() {
  var CleanCSS, DEP_ID_SUFFIX_REGEXP, EOL, EXPORTS_REGEXP, Q, UglifyJS, _, _bowerDir, _npmDir, _venderFoundMap, amdDependency, async, child_process, coffee, coffeeReactTransform, findVendor, findVendorInDir, fixBowerDir, fixDefineParams, fs, getBodyDeps, getUnixStylePath, gutil, logErr, mkdirp, mt2amd, path, reactTools, through, traceur;
  ...
  findVendorInDir = function(inDir, outDir, name, opt, callback) {
    var confFile, content, err, error, i, item, j, len, len1, mainMapped, mainPath, minifyJS, moduleDir, outPath, outPathExists, packageObj, packagePath, ref, ref1, ref2, ref3, stylePath;
    outDir = outDir.replace(/\{name\}/ig, name);
    moduleDir = path.resolve(inDir, name);
    mainMapped = (ref = opt.mainMap) != null ? ref[name] : void 0;
    if (mainMapped) {
    ...
    mainMapped = (ref1 = opt.mainMap) != null ? ref1[name + ".css"] : void 0;
    if (mainMapped) {
      if (mainMapped.indexOf('@') >= 0) {
        mainMapped = mainMapped.split('@');
        moduleDir = path.resolve(inDir, mainMapped[1]);
