(function() {
  var DEP_ID_SUFFIX_REGEXP, EOL, Q, UglifyJS, amdDependency, async, coffee, coffeeReactTransform, fs, gutil, logErr, mkdirp, mt2amd, path, reactTools, through, traceur, _, _bowerDir, _findVendor, _findVendorInDir, _fixBowerDir, _npmDir, _venderFoundMap;
  ...
  _fixBowerDir = function(inDir) {
    ...
    return _fixBowerDir = function() {};
  };

  _findVendor = function(inDir, outDir, name, opt, callback) {
    _fixBowerDir(inDir);
    return _findVendorInDir(path.resolve(inDir, _npmDir), outDir, name, opt, function(found) {
      if (found) {
        return callback();
      } else {
        return _findVendorInDir(path.resolve(inDir, _bowerDir), outDir, name, opt, function(found) {
          return callback();
        });
      }
    });
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      depStream.pipe(through.obj(function(file, enc, next) {
        ...
        return async.eachSeries(dependFiles, function(depFile, cb) {
          var coffeeStream, cwd, depContent, depId, fileName, findVendorOpt, inDir, outDir, reactOpt, requireBaseDir, trace, typeOfOpt;
