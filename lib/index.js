(function() {
  var CleanCSS, DEP_ID_SUFFIX_REGEXP, EOL, EXPORTS_REGEXP, PluginError, Q, UglifyJS, _, _bowerDir, _npmDir, _venderFoundMap, amdDependency, async, child_process, coffee, coffeeReactTransform, findVendor, findVendorInDir, fixBowerDir, fixDefineParams, fs, getBodyDeps, getUnixStylePath, logErr, mkdirp, mt2amd, path, through;
  ...
  findVendorInDir = function(inDir, outDir, depId, opt, callback) {
    var confFile, content, err, i, item, j, len, len1, mainMapped, mainPath, minifyJS, moduleDir, name, outId, outPath, outPathExists, packageObj, packagePath, ref, ref1, ref2, ref3, stylePath;
    name = depId.split('/')[0];
    if (opt.mkdir) {
      outDir = outDir + '/' + name;
    }
